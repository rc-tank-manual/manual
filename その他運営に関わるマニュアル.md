...
もうすでにRaspberry Pi ZEROで動くラジコン戦車があるのであれば、そこに搭載されているSDカードのデータをそのままイメージファイルに置き換えて、他のSDカードにコピーすればいいので、この章で説明する手順を行う必要はありません。

また、このサイトの[トップページ](http://ais.nsc.nagoya-cu.ac.jp)または[こちら](http://ais.nsc.nagoya-cu.ac.jp/RasPiTankZERO_20190806.zip)からRaspberry Pi ZERO用のラジコン戦車のイメージファイルをダウンロードすることもできます。

そのため、毎年、実験アシスタントや体験に来てくれた受講生が行う手順ではないということで、また別のマニュアルとしてこちらの記事を書くことにしました。
# ラズパイゼロでラジコン戦車プログラムを動かす方法
秀和システムが出版している『親子で電子工作入門&nbsp;ラズパイとスマホでラジコン戦車を作ろう！』をラズパイゼロで動かしてやろうというのが、この記事の趣旨です。
